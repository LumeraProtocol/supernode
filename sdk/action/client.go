...
type Client interface {
	...
	DownloadCascade(ctx context.Context, actionID, outputDir, signature, requesterAddress string) (string, error)
}
...
func (c *ClientImpl) DownloadCascade(ctx context.Context, actionID, outputDir, signature, requesterAddress string) (string, error) {

	if actionID == "" {
		return "", fmt.Errorf("actionID is empty")
	}

	taskID, err := c.taskManager.CreateDownloadTask(ctx, actionID, outputDir, signature, requesterAddress)
	if err != nil {
