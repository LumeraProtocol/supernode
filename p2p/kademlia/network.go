...
func (s *Network) Call(ctx context.Context, request *Message, isLong bool) (*Message, error) {
	...
	if isHighSignalMsg(request.MessageType) {
		logtrace.Info(ctx, "RPC call start", startFields)
	} else {
		logtrace.Debug(ctx, "RPC call start", startFields)
	}
...
func (s *Network) rpcOnceWrapper(ctx context.Context, cw *connWrapper, remoteAddr string, data []byte, timeout time.Duration, msgType int) (*Message, error) {
	...
	for {
		...
		if isHighSignalMsg(msgType) {
			logtrace.Info(ctx, "RPC ok", logtrace.Fields{logtrace.FieldModule: "p2p", "remote": remoteAddr, "message": msgName(msgType), "ms": time.Since(start).Milliseconds()})
		} else {
			logtrace.Debug(ctx, "RPC ok", logtrace.Fields{logtrace.FieldModule: "p2p", "remote": remoteAddr, "message": msgName(msgType), "ms": time.Since(start).Milliseconds()})
		}
		return r, nil
	}
}
