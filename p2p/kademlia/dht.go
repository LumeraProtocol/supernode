...
func (s *DHT) BatchRetrieve(ctx context.Context, keys []string, required int32, txID string, localOnly ...bool) (result map[string][]byte, err error) {
	...
	logtrace.Debug(ctx, "DHT BatchRetrieve complete", logtrace.Fields{"txid": txID, "found_local": foundLocalCount, "found_network": netFound, "required": required, "ms": time.Since(start).Milliseconds()})
...
func (s *DHT) doBatchGetValuesCall(ctx context.Context, node *Node, requestKeys map[string]KeyValWithClosest) (map[string]KeyValWithClosest, error) {
	...
	logtrace.Debug(ctx, "RPC BatchGetValues send", logtrace.Fields{"node": node.String(), "keys": len(requestKeys)})
	response, err := s.network.Call(ctx, request, false)
	if err != nil {
		return nil, fmt.Errorf("network call request %s failed: %w", request.String(), err)
	}
	logtrace.Debug(ctx, "RPC BatchGetValues completed", logtrace.Fields{"node": node.String()})
...
func (s *DHT) IterateBatchStore(ctx context.Context, values [][]byte, typ int, id string) error {
	...
	if requests > 0 {
		...
		if successRate >= minimumDataStoreSuccessRate {
			logtrace.Debug(ctx, "Successful store operations", logtrace.Fields{
				logtrace.FieldModule: "dht",
				"task_id":            id,
			...
			logtrace.Debug(ctx, "Failed to achieve desired success rate", logtrace.Fields{
				logtrace.FieldModule: "dht",
				"task_id":            id,
				"success_rate":       fmt.Sprintf("%.2f%%", successRate),
			})
	...
	for key, node := range nodes {
		...
		go func(receiver *Node, key string) {
			...
			default:
				...
				logtrace.Debug(ctx, "RPC BatchStoreData send", logtrace.Fields{logtrace.FieldModule: "dht", "node": receiver.String(), "keys": len(toStore), "size_mb": utils.BytesIntToMB(totalBytes)})
				...
				if err != nil {
					...
					logtrace.Debug(ctx, "RPC BatchStoreData failed", logtrace.Fields{logtrace.FieldModule: "p2p", logtrace.FieldError: err.Error(), "node": receiver.String(), "ms": dur})
					responses <- &MessageWithError{Error: err, Message: response, KeysCount: len(toStore), Receiver: receiver, DurationMS: dur}
					return
				}

				logtrace.Debug(ctx, "RPC BatchStoreData completed", logtrace.Fields{logtrace.FieldModule: "p2p", "node": receiver.String(), "keys": len(toStore), "ms": dur})
