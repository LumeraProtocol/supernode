...
import (
	...
	kademlia "github.com/LumeraProtocol/supernode/v2/p2p/kademlia"
)
...
func (_m *Client) StoreBatch(ctx context.Context, values [][]byte, typ int, taskID string) (float64, int, []kademlia.StoreCallMetric, error) {
	...
	var r2 []kademlia.StoreCallMetric
	if rf, ok := ret.Get(2).(func(context.Context, [][]byte, int, string) []kademlia.StoreCallMetric); ok {
		r2 = rf(ctx, values, typ, taskID)
	} else {
		if ret.Get(2) != nil {
			r2 = ret.Get(2).([]kademlia.StoreCallMetric)
		}
	}

	var r3 error
	if rf, ok := ret.Get(3).(func(context.Context, [][]byte, int, string) error); ok {
		r3 = rf(ctx, values, typ, taskID)
	} else {
		r3 = ret.Error(3)
	}
