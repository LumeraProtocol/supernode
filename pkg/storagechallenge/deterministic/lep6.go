...
// pairing for the entire active set when the target-candidate set is the active
// set. It is provided for tests and for callers who need the complete map (e.g.
// observability emit paths). For runtime use the chain's QueryAssignedTargets
// is the canonical source — call this only when chain access is unavailable or
// for cross-checking.
//
...
func AssignChallengerTargets(activeIDs, targets []string, seed []byte) map[string]string {
	return AssignChallengerTargetsWithCandidates(activeIDs, targets, activeIDs, seed)
}

// AssignChallengerTargetsWithCandidates mirrors Lumera's final
// audit_peer_assignment.go pairing loop, including the self-target fallback: if
// the selected target set has no available non-self target for a challenger, the
// chain scans the full ranked candidate set and picks the best non-self,
// unassigned candidate. This matters when a selected target would otherwise be
// assigned to itself.
func AssignChallengerTargetsWithCandidates(activeIDs, selectedTargets, targetCandidates []string, seed []byte) map[string]string {
	if len(activeIDs) == 0 || len(selectedTargets) == 0 {
		return map[string]string{}
	}
	challengers := sortedUniqueCopy(activeIDs)
	rankedCandidates := rankAccounts(seed, sortedUniqueCopy(targetCandidates), domainTargetRank)
	if len(rankedCandidates) == 0 {
		rankedCandidates = rankAccounts(seed, challengers, domainTargetRank)
	}

	assigned := make(map[string]struct{}, len(selectedTargets))
	unassignedSelected := make(map[string]struct{}, len(selectedTargets))
	for _, target := range selectedTargets {
		if target != "" {
			unassignedSelected[target] = struct{}{}
		}
	}

	uniqueTargetCount := len(unassignedSelected)
	out := make(map[string]string, len(challengers))
	for _, challenger := range challengers {
		if len(assigned) >= uniqueTargetCount {
			break
		}

		bestTarget := ""
		var bestRank []byte
		for target := range unassignedSelected {
			if target == challenger {
				continue
			}
			rank := storageTruthAssignmentHash(seed, challenger, target, domainPairRank)
			if bestTarget == "" || compareBytes(rank, bestRank) < 0 || (compareBytes(rank, bestRank) == 0 && target < bestTarget) {
				bestTarget = target
				bestRank = rank
			}
		}
		...
		if bestTarget == "" {
			for _, target := range rankedCandidates {
				if _, alreadyAssigned := assigned[target.id]; alreadyAssigned || target.id == challenger {
					continue
				}
				rank := storageTruthAssignmentHash(seed, challenger, target.id, domainPairRank)
				if bestTarget == "" || compareBytes(rank, bestRank) < 0 || (compareBytes(rank, bestRank) == 0 && target.id < bestTarget) {
		...
		delete(unassignedSelected, bestTarget)
		assigned[bestTarget] = struct{}{}
		out[challenger] = bestTarget
	}
	return out
}
...
// LEP-6 §9 step 2).
//
// Rank: SHA-256(seed || 0x00 || target || 0x00 || bucket || 0x00 || ticket_id || 0x00 || "ticket_rank")
...
func SelectTicketForBucket(eligibleTicketIDs []string, excluded map[string]struct{}, seed []byte, target string, bucket audittypes.StorageProofBucketType) string {
	...
	for _, t := range eligibleTicketIDs {
		...
		rank := storageTruthAssignmentHash(seed, target, bucketStr, t, domainTicketRank)
		if bestTicket == "" {
			bestTicket = t
			bestRank = rank
			continue
...
// schema unambiguous and length-stable. If you change to decimal, you must
// version the domain separator and update the protocol guide.
