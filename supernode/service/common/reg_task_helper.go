...
// 	splits := bytes.Split(decData, []byte{lumera.SeparatorByte})
...
// 			err := lumeraClient.Verify(ctx, string(splits[i]), file, pastelIDs[j])
// 			if err != nil {
...
// 		defer close(ch)
// 		blockTracker := blocktracker.New(h.LumeraHandler)
...
// 				txResult, err := h.LumeraHandler.GetRawTransactionVerbose1(gctx, txid)
// 				if err != nil {
...
// 			if addr == h.LumeraHandler.GetBurnAddress() {
// 				isTxnAddressOk = true
// 			}
// 		}
...
// 			h.LumeraHandler.GetBurnAddress(), string(data), totalAmt, reqBurnAmount)
// 	}
...
// 		if ok, err := h.LumeraHandler.Verify(ctx, data, string(signature), nodeID, pastel.SignAlgorithmED448); err != nil {
...
// 	for nodeID, signature := range h.PeersTicketSignature {
// 		if ok, err := h.LumeraHandler.VerifyCollectionTicket(ctx, data, string(signature), nodeID, pastel.SignAlgorithmED448); err != nil {
// 			return errors.Errorf("verify signature %s of node %s", signature, nodeID)
// 		} else if !ok {
// 			return errors.Errorf("signature of node %s mistmatch", nodeID)
// 		}
