...
flowchart TD
  ...
  F --> G{Am I leader?}
  G -- No --> E
  G -- Yes --> H["List DONE or APPROVED cascade actions"]
  ...
  K -- Yes --> M["Pick deterministic recipient and observers"]
  M --> N["Persist challenge events and metrics"]
```
...
Self-healing E2E now covers:

1. Happy path: request -> verify -> commit (hash and persistence assertions)
2. Observer tampered-hash rejection
3. Stale epoch rejection (`RequestSelfHealing`, `VerifySelfHealing`, `CommitSelfHealing`)
4. Duplicate challenge replay behavior
5. Recipient-down request failure

CI integration:

- workflow job: `self-healing-e2e-tests` (`.github/workflows/tests.yml`)
- command: `make test-self-healing`
- make target runs all self-healing E2E tests with:
  - `go test -tags=system_test -v -run '^TestSelfHealingE2E' .`
