...
type reseedReq struct {
	...
	MaxSymbols int    `json:"max_symbols,omitempty"` // 0 = all
}
...
func (ra *recoveryAdmin) handleReseed(w http.ResponseWriter, r *http.Request) {
	...
	// 1) First, verify this action is fully decodable (same spirit as current download flow).
	var tmpDir string
	var events int
	var lastEvent string
	err := crt.Download(ctx, &cascadeService.DownloadRequest{
	...
	if tmpDir != "" {
		_ = crt.CleanupDownload(ctx, tmpDir)
	}
	if err != nil {
		writeJSON(w, http.StatusOK, map[string]any{
			"ok":                          false,
	...
	// 2) Resolve artefacts (index/layout/symbol keys) from chain metadata + P2P.
	bundle, err := ra.resolveArtefactBundle(ctx, req.ActionID)
	if err != nil {
		writeJSON(w, http.StatusOK, map[string]any{
			"ok":           false,
			"action_id":    req.ActionID,
	...
	// 3) Active set policy for reseed: only active set is considered the replication target set.
	...
	// 4) Re-publish index + layout payloads.
	indexLayoutPayload := make([][]byte, 0, len(bundle.IndexPayload)+len(bundle.LayoutRaw))
	indexLayoutPayload = append(indexLayoutPayload, bundle.IndexPayload...)
	indexLayoutPayload = append(indexLayoutPayload, bundle.LayoutRaw...)
	if len(indexLayoutPayload) > 0 {
		if err := ra.p2pClient.StoreBatch(ctx, indexLayoutPayload, 1, req.ActionID); err != nil {
			writeJSON(w, http.StatusOK, map[string]any{"ok": false, "action_id": req.ActionID, "error": fmt.Sprintf("store index/layout batch: %v", err)})
			return
		}
	}

	// 5) Re-publish symbols.
	keys := bundle.SymbolKeys
	if req.MaxSymbols > 0 && req.MaxSymbols < len(keys) {
		keys = keys[:req.MaxSymbols]
	}
	var (
		symbolsRepublished int
		curBatch           = make([][]byte, 0, 100)
	)
	onSymbol := func(_ string, data []byte) error {
		if len(data) == 0 {
			return nil
		}
		curBatch = append(curBatch, data)
		symbolsRepublished++
		if len(curBatch) >= 100 {
			if err := ra.p2pClient.StoreBatch(ctx, curBatch, 1, req.ActionID); err != nil {
				return err
			}
			curBatch = curBatch[:0]
		}
	...
	if len(keys) > 0 {
		if _, err := ra.p2pClient.BatchRetrieveStream(ctx, keys, int32(len(keys)), req.ActionID, onSymbol); err != nil {
			writeJSON(w, http.StatusOK, map[string]any{"ok": false, "action_id": req.ActionID, "error": fmt.Sprintf("retrieve symbols for reseed: %v", err)})
			return
		}
	}
	if len(curBatch) > 0 {
		if err := ra.p2pClient.StoreBatch(ctx, curBatch, 1, req.ActionID); err != nil {
			writeJSON(w, http.StatusOK, map[string]any{"ok": false, "action_id": req.ActionID, "error": fmt.Sprintf("store symbols batch: %v", err)})
			return
		}
	}

	// 6) Probe only active set for post-reseed visibility.
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"events":                      events,
		"last_event":                  lastEvent,
		"index_files_expected":        len(bundle.IndexIDs),
		"index_files_republished":     len(bundle.IndexPayload),
		"layout_files_expected":       len(bundle.LayoutIDs),
		"layout_files_republished":    len(bundle.LayoutRaw),
		"symbols_expected":            len(keys),
		"symbols_republished":         symbolsRepublished,
...
func (ra *recoveryAdmin) handleStatus(w http.ResponseWriter, r *http.Request, actionID string) {
	...
	writeJSON(w, http.StatusOK, map[string]any{
		...
		"summary": map[string]any{
			"supernodes_checked":   len(results),
