...
func (hm *Collector) collectMetrics(ctx context.Context) (sntypes.SupernodeMetrics, error) {
	...
	// Include P2P metrics so `peers_count` reflects actual network state; on-chain
	// validation requires `peers_count > 0` for compliant reports.
	statusResp, err := hm.statusService.GetStatus(ctx, true)
	if err != nil {
