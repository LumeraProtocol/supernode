...
// ConnectSN implements node.Client.ConnectSN() to connect nodes for storage-challenges/self-healing/metrics etc only
func (client *client) ConnectSN(ctx context.Context, address string) (node.ConnectionInterface, error) {
	grpclog.SetLoggerV2(log.NewLoggerWithErrorLevel())
	...
	if err != nil {
		log.WithContext(ctx).WithError(err).Debug("DialContext err")
		return nil, errors.Errorf("dial address %s: %w", address, err)
	}
