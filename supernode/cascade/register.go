...
func (task *CascadeRegistrationTask) Register(
    ...
    if err := task.verifyActionFee(ctx, action, req.DataSize, fields); err != nil { return err }
    ...
    if err := task.ensureIsTopSupernode(ctx, uint64(action.BlockHeight), fields); err != nil { return err }
    ...
    if err != nil { return task.wrapErr(ctx, "failed to unmarshal cascade metadata", err, fields) }
    ...
    if err := cascadekit.VerifyB64DataHash(req.DataHash, cascadeMeta.DataHash); err != nil { return err }
    ...
    if err := cascadekit.VerifySingleBlockIDs(layout, encResp.Metadata); err != nil { return task.wrapErr(ctx, "failed to verify IDs", err, fields) }
    ...
    if err := task.storeArtefacts(ctx, action.ActionID, rqidResp.RedundantMetadataFiles, encResp.SymbolsDir, fields); err != nil { return err }
