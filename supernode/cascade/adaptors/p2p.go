...
type P2PService interface { StoreArtefacts(ctx context.Context, req StoreArtefactsRequest, f logtrace.Fields) error }

type p2pImpl struct { p2p p2p.Client; rqStore rqstore.Store }

func NewP2PService(client p2p.Client, store rqstore.Store) P2PService { return &p2pImpl{p2p: client, rqStore: store} }

type StoreArtefactsRequest struct { TaskID string; ActionID string; IDFiles [][]byte; SymbolsDir string }
...
func (p *p2pImpl) StoreArtefacts(ctx context.Context, req StoreArtefactsRequest, f logtrace.Fields) error {
    ...
    if err != nil { return fmt.Errorf("error storing artefacts: %w", err) }
    _ = firstPassSymbols; _ = totalSymbols; _ = start
    remaining := 0
    if req.SymbolsDir != "" { if keys, werr := walkSymbolTree(req.SymbolsDir); werr == nil { remaining = len(keys) } }
    ...
    if remaining == 0 { logtrace.Info(ctx, "store: dir empty after first-pass", logtrace.Fields{"taskID": req.TaskID, "dir": req.SymbolsDir}) }
    return nil
}
...
func (p *p2pImpl) storeCascadeSymbolsAndData(ctx context.Context, taskID, actionID string, symbolsDir string, metadataFiles [][]byte) (int, int, error) {
    if err := p.rqStore.StoreSymbolDirectory(taskID, symbolsDir); err != nil { return 0, 0, fmt.Errorf("store symbol dir: %w", err) }
    keys, err := walkSymbolTree(symbolsDir); if err != nil { return 0, 0, err }
    totalAvailable := len(keys)
    targetCount := int(math.Ceil(float64(totalAvailable) * storeSymbolsPercent / 100.0))
    if targetCount < 1 && totalAvailable > 0 { targetCount = 1 }
    ...
    if len(keys) > loadSymbolsBatchSize {
        want := targetCount
        if want < len(keys) { rand.Shuffle(len(keys), func(i, j int) { keys[i], keys[j] = keys[j], keys[i] }); keys = keys[:want] }
        sort.Strings(keys)
    }
    ...
    for start := 0; start < len(keys); {
        ...
        if !firstBatchProcessed && len(metadataFiles) > 0 {
            ...
            if roomForSymbols < 0 { roomForSymbols = 0 }
            if roomForSymbols < len(batch) { batch = batch[:roomForSymbols]; end = start + roomForSymbols }
            symBytes, err := utils.LoadSymbols(symbolsDir, batch); if err != nil { return 0, 0, fmt.Errorf("load symbols: %w", err) }
            payload := make([][]byte, 0, len(metadataFiles)+len(symBytes)); payload = append(payload, metadataFiles...); payload = append(payload, symBytes...)
            ...
            bctx, cancel := context.WithTimeout(ctx, storeBatchContextTimeout); err = p.p2p.StoreBatch(bctx, payload, P2PDataRaptorQSymbol, taskID); cancel(); if err != nil { return totalSymbols, totalAvailable, fmt.Errorf("p2p store batch (first): %w", err) }
            ...
            if len(batch) > 0 { if err := utils.DeleteSymbols(ctx, symbolsDir, batch); err != nil { return totalSymbols, totalAvailable, fmt.Errorf("delete symbols: %w", err) } }
            firstBatchProcessed = true
        } else {
            count, err := p.storeSymbolsInP2P(ctx, taskID, symbolsDir, batch); if err != nil { return totalSymbols, totalAvailable, err }
            totalSymbols += count
        }
        start = end
    }
    if err := p.rqStore.UpdateIsFirstBatchStored(actionID); err != nil { return totalSymbols, totalAvailable, fmt.Errorf("update first-batch flag: %w", err) }
...
func walkSymbolTree(root string) ([]string, error) {
    ...
    err := filepath.WalkDir(root, func(path string, d fs.DirEntry, err error) error {
        ...
        if strings.EqualFold(filepath.Ext(d.Name()), ".json") { return nil }
        rel, err := filepath.Rel(root, path); if err != nil { return err }
        keys = append(keys, rel)
        return nil
    })
    if err != nil { return nil, fmt.Errorf("walk symbol tree: %w", err) }
    return keys, nil
}
...
func (c *p2pImpl) storeSymbolsInP2P(ctx context.Context, taskID, root string, fileKeys []string) (int, error) {
    ...
    symbols, err := utils.LoadSymbols(root, fileKeys); if err != nil { return 0, fmt.Errorf("load symbols: %w", err) }
    symCtx, cancel := context.WithTimeout(ctx, storeBatchContextTimeout); defer cancel()
    logtrace.Info(ctx, "store: batch send (symbols)", logtrace.Fields{"taskID": taskID, "symbols_in_batch": len(symbols)})
    if err := c.p2p.StoreBatch(symCtx, symbols, P2PDataRaptorQSymbol, taskID); err != nil { return len(symbols), fmt.Errorf("p2p store batch: %w", err) }
    logtrace.Info(ctx, "store: batch ok (symbols)", logtrace.Fields{"taskID": taskID, "symbols_stored": len(symbols)})
    if err := utils.DeleteSymbols(ctx, root, fileKeys); err != nil { return len(symbols), fmt.Errorf("delete symbols: %w", err) }
